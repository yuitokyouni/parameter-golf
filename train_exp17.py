# exp17: Full GDN (Gated DeltaNet) — all 6 blocks use GDN attention
# Weight-Tied 6L×2, d=512, SmearGate, BigramHash, EMA
# GDN replaces CausalSelfAttention; XSA disabled (incompatible with GDN)
...
class Hyperparameters:
    ...
    # Per-layer matrix cost: 9*d^2  (4.06M @ 672; 4.26M @ 688).
    # 6 unique layers: 24.39M @ 672 ; 25.56M @ 688  (vs 25.95M for 11L@512 baseline).
    ...
    xsa_last_n = int(os.environ.get("XSA_LAST_N", 0))  # XSA disabled (incompatible with GDN)
    rope_dims = int(os.environ.get("ROPE_DIMS", 0))  # GDN handles its own position encoding
    gdn_num_heads = int(os.environ.get("GDN_NUM_HEADS", 8))
    attention_types = os.environ.get("ATTENTION_TYPES", "gdn,gdn,gdn,gdn,gdn,gdn")  # per-block
...
class GDNAttention(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int):
        ...
        self.use_xsa = False  # GDN doesn't support XSA
    def forward(self, x: Tensor, **kwargs) -> tuple[Tensor, None]:
        return self.gdn(x), None  # (output, raw_v=None for XSA compat)
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        attention_types: str = "gdn,gdn,gdn,gdn,gdn,gdn",
        gdn_num_heads: int = 8,
    ):
        super().__init__()
        ...
        # Parse per-block attention types
        self.block_attn_types = [t.strip() for t in attention_types.split(",")]
        if len(self.block_attn_types) != num_layers:
            raise ValueError(f"attention_types has {len(self.block_attn_types)} entries but num_layers={num_layers}")
        self.swa_indices = [i for i, t in enumerate(self.block_attn_types) if t == 'swa']
        self.gdn_indices = [i for i, t in enumerate(self.block_attn_types) if t == 'gdn']
        num_swa = len(self.swa_indices)
        # Parameter banks: only for SWA blocks' attention weights
        head_dim = model_dim // num_heads
        kv_dim = num_kv_heads * head_dim
        mlp_dim = int(mlp_mult * model_dim)
        self.num_layers = num_layers
        self.num_swa = num_swa
        if num_swa > 0:
            ...
            # Register as buffers so state_dict is consistent but no params to optimize
            self.register_buffer('qo_bank', torch.empty(0))
            self.register_buffer('kv_bank', torch.empty(0))
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    attention_type=self.block_attn_types[i],
    ...
    def _init_weights(self) -> None:
        ...
        # Init SWA attention banks
        for j, i in enumerate(self.swa_indices):
            nn.init.orthogonal_(self.qo_bank.data[j], gain=1.0)
            nn.init.zeros_(self.qo_bank.data[num_swa + j])
            nn.init.orthogonal_(self.kv_bank.data[j], gain=1.0)
            nn.init.orthogonal_(self.kv_bank.data[num_swa + j], gain=1.0)
            self.qo_bank.data[num_swa + j].mul_(proj_scale)
        # Init MLP banks (all blocks)
        for i in range(n):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        # Build SWA block-index -> bank-index mapping
        swa_bank_map = {i: j for j, i in enumerate(self.swa_indices)}
        # Pass 1 (encoder): run all `n` unique blocks, push skips.
        for i in range(self.num_encoder_layers):
            ve = self._get_ve(i, input_ids, ve_cache)
            if self.block_attn_types[i] == 'gdn':
                x, raw_v = self.blocks[i](x, x0, None, None, None, None,
                    self.mlp_up_bank[i], self.mlp_down_bank[i])
            else:
                j = swa_bank_map[i]
                x, raw_v = self.blocks[i](x, x0,
                    self.qo_bank[j], self.kv_bank[j], self.kv_bank[num_swa + j],
                    self.qo_bank[num_swa + j], self.mlp_up_bank[i], self.mlp_down_bank[i],
                    v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            if self.block_attn_types[i] == 'gdn':
                x, _ = self.blocks[i](x, x0, None, None, None, None,
                    self.mlp_up_bank[i], self.mlp_down_bank[i])
            else:
                j = swa_bank_map[i]
...
def _unbank_state_dict(sd: dict[str, Tensor], num_layers: int, swa_indices: list[int] | None = None) -> dict[str, Tensor]:
    ...
    out: dict[str, Tensor] = {}
    n = num_layers
    swa_set = set(swa_indices) if swa_indices is not None else set(range(n))
    num_swa = len(swa_set)
    swa_list = sorted(swa_set)
    for name, tensor in sd.items():
        if name == "qo_bank":
            if tensor.numel() == 0:
                continue
            for j, i in enumerate(swa_list):
                out[f"blocks.{i}.attn.c_q.weight"] = tensor[j]
                out[f"blocks.{i}.attn.proj.weight"] = tensor[num_swa + j]
        elif name == "kv_bank":
            if tensor.numel() == 0:
                continue
            for j, i in enumerate(swa_list):
                out[f"blocks.{i}.attn.c_k.weight"] = tensor[j]
                out[f"blocks.{i}.attn.c_v.weight"] = tensor[num_swa + j]
...
def _rebank_state_dict(sd: dict[str, Tensor], num_layers: int, template_sd: dict[str, Tensor], swa_indices: list[int] | None = None) -> dict[str, Tensor]:
    ...
    if num_swa > 0:
        ...
        out["qo_bank"] = template_sd["qo_bank"].clone()
        out["kv_bank"] = template_sd["kv_bank"].clone()
    up_slices = [None] * n
    down_slices = [None] * n
    for i in range(n):
...
class _HessianGPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
                 ...
                 attention_types="gdn,gdn,gdn,gdn,gdn,gdn", gdn_num_heads=8):
        super().__init__()
        self.tie_embeddings = tie_embeddings
        self.logit_softcap = logit_softcap
        self.num_layers = num_layers
        ...
        self.blocks = nn.ModuleList([
            _HessianBlock(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                          layer_idx=i, ln_scale=ln_scale, attention_type=self.block_attn_types[i],
                          gdn_num_heads=gdn_num_heads)
            for i in range(num_layers)
        ])
        if rope_dims > 0:
...
def main() -> None:
    ...
    # - MLP banks -> Muon
    # - GDN 2D params -> Muon
    # - SWA attention banks -> Muon (if any)
    ...
    # Add GDN 2D parameters to Muon, collect all GDN param ids
    gdn_matrix_param_set = set()
    gdn_all_param_set = set()
    for i in base_model.gdn_indices:
        for name, p in base_model.blocks[i].attn.gdn.named_parameters():
            gdn_all_param_set.add(id(p))
            if p.ndim == 2 and p.shape[0] >= 64 and p.shape[1] >= 64:
                matrix_params.append(p)
                gdn_matrix_param_set.add(id(p))
    block_named_params = list(base_model.blocks.named_parameters())
    scalar_params = [
        p
        for name, p in block_named_params
        if (p.ndim < 2 or any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS))
           and id(p) not in gdn_all_param_set
    ]
    # Collect GDN 1D parameters for scalar optimizer
    gdn_scalar_params = []
    for i in base_model.gdn_indices:
        for name, p in base_model.blocks[i].attn.gdn.named_parameters():
            if id(p) not in gdn_matrix_param_set:
                gdn_scalar_params.append(p)
    scalar_params.extend(gdn_scalar_params)
    ...
    # GDN params need manual all-reduce (not handled by Muon's reduce-scatter)
    for i in base_model.gdn_indices:
        for p in base_model.blocks[i].attn.gdn.parameters():
            replicated_params.append(p)
    ...
    attn_config = ", ".join(f"block{i}={base_model.block_attn_types[i]}" for i in range(args.num_layers))
    log0(f"attention_config: [{attn_config}]")
    log0(f"gdn_blocks:{len(base_model.gdn_indices)} swa_blocks:{len(base_model.swa_indices)}")
    # Estimate artifact size
    total_params = sum(p.numel() for p in base_model.parameters())
    est_bytes = total_params * 0.75 + 10000  # rough int6 estimate
    log0(f"model_params:{total_params} estimated_int6_artifact:{est_bytes/1024/1024:.1f}MB")
