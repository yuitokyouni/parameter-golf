# exp19: Hybrid GDN + wider dimension — 5 GDN + 1 SWA, d=640
# Weight-Tied 6L×2, d=640 (fallback to d=576 if >16MB), SmearGate, BigramHash, EMA
# Blocks 0-4: GDN (Gated DeltaNet), Block 5: SWA with XSA
# Based on train_exp13.py (1.1639 bpb baseline)
...
try:
    ...
    _GDN_AVAILABLE = True
    print("GDN: flash-linear-attention imported successfully")
except ImportError:
    print("ERROR: flash-linear-attention not installed. Run: pip install flash-linear-attention")
    sys.exit(1)
# --- Attention backend shim ---
# The 1st-place record imports flash_attn_3_func from flash_attn_interface (Hopper only).
# We provide an SDPA-based fallback so this runs on Ada (RTX 4090) and other non-Hopper GPUs.
# Call-site signature (kept identical): flash_attn_3_func(q, k, v, causal=True),
# where q is [B, T, H, D] and k, v are [B, T, Hkv, D]. Return is [B, T, H, D].
try:
    from flash_attn_interface import flash_attn_func as _flash_attn_3_func  # type: ignore
    ...
    def flash_attn_3_func(q, k, v, causal: bool = True):
        ...
        k_ = k.transpose(1, 2)  # -> [B, Hkv, T, D]
        v_ = v.transpose(1, 2)
        Hq, Hkv = q_.size(1), k_.size(1)
        if Hq != Hkv:
            # Expand GQA manually (enable_gqa kwarg is PyTorch >=2.5 only).
            rep = Hq // Hkv
...
class Hyperparameters:
    ...
    # exp19: d=640 (fallback to d=576 via MODEL_DIM=576 env var if >16MB).
    # num_heads=8 => head_dim=80 at 640. 5 GDN + 1 SWA hybrid architecture.
    ...
    xsa_last_n = int(os.environ.get("XSA_LAST_N", 1))  # XSA on last SWA block only
    rope_dims = int(os.environ.get("ROPE_DIMS", 16))
    gdn_num_heads = int(os.environ.get("GDN_NUM_HEADS", 8))
    attention_types = os.environ.get("ATTENTION_TYPES", "gdn,gdn,gdn,gdn,gdn,swa")
...
class GDNAttention(nn.Module):
    """Wrapper around fla's GatedDeltaNet to match Block interface."""
    def __init__(self, dim: int, num_heads: int):
        super().__init__()
        self.gdn = GatedDeltaNet(hidden_size=dim, num_heads=num_heads, mode='chunk')
        self.use_xsa = False
    def forward(self, x: Tensor, **kwargs) -> tuple[Tensor, None]:
        return self.gdn(x), None
...
class Block(nn.Module):
    def __init__(
        ...
        if attention_type == 'gdn':
            self.attn = GDNAttention(dim, gdn_num_heads)
        else:
        ...
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor = None, k_w: Tensor = None, v_w: Tensor = None, out_w: Tensor = None, up_w: Tensor = None, down_w: Tensor = None, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        attention_types: str = "swa,swa,swa,swa,swa,swa",
        gdn_num_heads: int = 8,
    ):
        super().__init__()
        ...
        attn_types = [t.strip() for t in attention_types.split(",")]
        if len(attn_types) != num_layers:
            raise ValueError(f"attention_types has {len(attn_types)} entries but num_layers={num_layers}")
        self.swa_indices = [i for i, t in enumerate(attn_types) if t == 'swa']
        self.gdn_indices = [i for i, t in enumerate(attn_types) if t == 'gdn']
        self.num_swa = len(self.swa_indices)
        ...
        # Parameter banks: contiguous 3D tensors for batched optimizer (SWA blocks only)
        ...
        if num_swa > 0:
            self.qo_bank = nn.Parameter(torch.empty(2 * num_swa, model_dim, model_dim))
            self.kv_bank = nn.Parameter(torch.empty(2 * num_swa, kv_dim, model_dim))
        else:
            self.register_parameter('qo_bank', None)
            self.register_parameter('kv_bank', None)
        self.mlp_up_bank = nn.Parameter(torch.empty(num_layers, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(num_layers, model_dim, mlp_dim))
        # Map from SWA block index to bank slot
        self._swa_to_bank = {idx: slot for slot, idx in enumerate(self.swa_indices)}
        ...
        if rope_dims > 0:
            head_dim = model_dim // num_heads
            for i in self.swa_indices:
                self.blocks[i].attn.rope_dims = rope_dims
                self.blocks[i].attn.rotary = Rotary(head_dim, base=rope_base, train_seq_len=1024, rope_dims=rope_dims)
        ...
        if xsa_last_n > 0:
            # Only apply XSA to the last xsa_last_n SWA blocks
            swa_for_xsa = self.swa_indices[-xsa_last_n:] if len(self.swa_indices) >= xsa_last_n else self.swa_indices
            for i in swa_for_xsa:
    ...
    def _init_weights(self) -> None:
        ...
        # Init attention banks: only for SWA blocks
        if num_swa > 0:
            for slot, idx in enumerate(self.swa_indices):
                nn.init.orthogonal_(self.qo_bank.data[slot], gain=1.0)           # Q
                nn.init.zeros_(self.qo_bank.data[num_swa + slot])                 # Out (zero init)
                nn.init.orthogonal_(self.kv_bank.data[slot], gain=1.0)           # K
                nn.init.orthogonal_(self.kv_bank.data[num_swa + slot], gain=1.0) # V
                self.qo_bank.data[num_swa + slot].mul_(proj_scale)
        # Init MLP banks for all blocks
        for i in range(n):
    ...
    def _block_forward(self, i: int, x: Tensor, x0: Tensor, input_ids: Tensor, ve_cache: dict, v0: Tensor | None, ve_pos: int) -> tuple[Tensor, Tensor | None]:
        """Dispatch block forward based on attention type (GDN or SWA)."""
        ve = self._get_ve(ve_pos, input_ids, ve_cache)
        if self.blocks[i].attention_type == 'gdn':
            return self.blocks[i](x, x0,
                up_w=self.mlp_up_bank[i], down_w=self.mlp_down_bank[i],
                v_embed=ve, v0=v0)
        else:
            slot = self._swa_to_bank[i]
            ns = self.num_swa
            return self.blocks[i](x, x0,
                self.qo_bank[slot], self.kv_bank[slot], self.kv_bank[ns + slot],
                self.qo_bank[ns + slot], self.mlp_up_bank[i], self.mlp_down_bank[i],
                v_embed=ve, v0=v0)
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            x, raw_v = self._block_forward(i, x, x0, input_ids, ve_cache, v0, ve_pos=i)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self._block_forward(i, x, x0, input_ids, ve_cache, v0, ve_pos=pos)
        x = self.final_norm(x)
...
def _unbank_state_dict(sd: dict[str, Tensor], num_layers: int, swa_indices: list[int] | None = None) -> dict[str, Tensor]:
    """Convert 3D bank tensors into individual 2D tensors with standard names."""
    out: dict[str, Tensor] = {}
    n = num_layers
    # If swa_indices not provided, assume all blocks are SWA (backward compat)
    swa_idx = swa_indices if swa_indices is not None else list(range(n))
    num_swa = len(swa_idx)
    for name, tensor in sd.items():
        if name == "qo_bank" and tensor is not None:
            for slot, block_i in enumerate(swa_idx):
                out[f"blocks.{block_i}.attn.c_q.weight"] = tensor[slot]
                out[f"blocks.{block_i}.attn.proj.weight"] = tensor[num_swa + slot]
        elif name == "kv_bank" and tensor is not None:
            for slot, block_i in enumerate(swa_idx):
                out[f"blocks.{block_i}.attn.c_k.weight"] = tensor[slot]
                out[f"blocks.{block_i}.attn.c_v.weight"] = tensor[num_swa + slot]
...
def _rebank_state_dict(sd: dict[str, Tensor], num_layers: int, template_sd: dict[str, Tensor], swa_indices: list[int] | None = None) -> dict[str, Tensor]:
    """Convert individual 2D tensors back into 3D bank tensors."""
    out: dict[str, Tensor] = {}
    n = num_layers
    swa_idx = swa_indices if swa_indices is not None else list(range(n))
    num_swa = len(swa_idx)
    # Reconstruct attention banks from SWA block weight keys
    consumed = set()
    if num_swa > 0 and "qo_bank" in template_sd and template_sd["qo_bank"] is not None:
        qo_slices = [None] * (2 * num_swa)
        kv_slices = [None] * (2 * num_swa)
        for slot, block_i in enumerate(swa_idx):
            qk = f"blocks.{block_i}.attn.c_q.weight"
            if qk in sd:
                qo_slices[slot] = sd[qk]
                consumed.add(qk)
            ok = f"blocks.{block_i}.attn.proj.weight"
            if ok in sd:
                qo_slices[num_swa + slot] = sd[ok]
                consumed.add(ok)
            kk = f"blocks.{block_i}.attn.c_k.weight"
            if kk in sd:
                kv_slices[slot] = sd[kk]
                consumed.add(kk)
            vk = f"blocks.{block_i}.attn.c_v.weight"
            if vk in sd:
                kv_slices[num_swa + slot] = sd[vk]
                consumed.add(vk)
    ...
    # Reconstruct MLP banks for all blocks
    up_slices = [None] * n
    down_slices = [None] * n
    for i in range(n):
...
class _HessianBlock(nn.Module):
    def __init__(self, dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, layer_idx=0, ln_scale=False, attention_type='swa', gdn_num_heads=8):
        super().__init__()
        self.attention_type = attention_type
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
    ...
    def forward(self, x, x0, v_embed=None):
        ...
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        if self.attention_type == 'gdn':
            attn_out, _ = self.attn(self.attn_norm(x_in) * self.ln_scale_factor)
        else:
...
class _HessianGPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
                 ...
                 attention_types="swa,swa,swa,swa,swa,swa", gdn_num_heads=8):
        super().__init__()
        self.tie_embeddings = tie_embeddings
        self.logit_softcap = logit_softcap
        self.num_layers = num_layers
        ...
        self.blocks = nn.ModuleList([
            _HessianBlock(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                          layer_idx=i, ln_scale=ln_scale, attention_type=attn_types[i], gdn_num_heads=gdn_num_heads)
            for i in range(num_layers)
        ])
        if rope_dims > 0:
            head_dim = model_dim // num_heads
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        gdn_num_heads=args.gdn_num_heads,
    ).to(device).bfloat16()
    # Banks stay FP32 (like CastedLinear weights), cast to BF16 in forward
    if base_model.qo_bank is not None:
        base_model.qo_bank.data = base_model.qo_bank.data.float()
    if base_model.kv_bank is not None:
    ...
    # Size estimation: check if d=640 fits in 16MB
    n_params_est = sum(p.numel() for p in base_model.parameters())
    est_int6_bytes = int(n_params_est * 0.75)  # 6 bits per param
    est_overhead = 200_000  # lzma metadata, scales, code
    est_code_bytes = len(code.encode("utf-8"))
    est_total = est_int6_bytes + est_overhead + est_code_bytes
    target_bytes_est = 16 * 1024 * 1024
    log0(f"size_estimate: params={n_params_est} est_int6={est_int6_bytes} est_total={est_total/(1024*1024):.2f}MB target=16.0MB")
    if est_total > target_bytes_est:
        log0(f"WARNING: estimated artifact ({est_total/(1024*1024):.2f}MB) exceeds 16MB!")
        log0(f"Consider reducing MODEL_DIM from {args.model_dim} to 576")
    ...
    if base_model.num_swa > 0:
        matrix_params = [base_model.qo_bank, base_model.kv_bank] + matrix_params
    gdn_muon_params = []
    for i in base_model.gdn_indices:
        for name, p in base_model.blocks[i].attn.gdn.named_parameters():
            if p.ndim == 2 and min(p.shape) >= 64:
                gdn_muon_params.append(p)
    matrix_params.extend(gdn_muon_params)
    gdn_muon_ids = {id(p) for p in gdn_muon_params}

    block_named_params = list(base_model.blocks.named_parameters())
    scalar_params = [
        p
        for name, p in block_named_params
        if id(p) not in gdn_muon_ids
        and (p.ndim < 2 or any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)
             or (p.ndim == 2 and min(p.shape) < 64))  # small GDN matrices -> Adam
    ]
    ...
    log0(f"attention_types:{args.attention_types} gdn_indices:{base_model.gdn_indices} swa_indices:{base_model.swa_indices}")
    log0(f"gdn_num_heads:{args.gdn_num_heads} gdn_muon_params:{len(gdn_muon_params)}")
    xsa_layers = [i for i, b in enumerate(base_model.blocks) if hasattr(b.attn, 'use_xsa') and b.attn.use_xsa]
    log0(f"XSA:last_{args.xsa_last_n} active_unique_blocks:{xsa_layers} (each block runs twice per forward)")
    log0(f"weight_tied:num_unique_blocks={args.num_layers} effective_depth={2*args.num_layers} model_dim={args.model_dim} ln_scale_schedule=cached_per_block(exp11)")
    ...
    unbanked_sd = _unbank_state_dict(sd_cpu, args.num_layers, swa_indices=base_model.swa_indices)
    ...
    hessian_model = _HessianGPT(
        ...
        attention_types=args.attention_types, gdn_num_heads=args.gdn_num_heads,
    ).to(device).bfloat16()
    ...
    deq_state = _rebank_state_dict(deq_unbanked, args.num_layers, sd_cpu, swa_indices=base_model.swa_indices)
    eval_model = GPT(
        vocab_size=args.vocab_size, num_layers=args.num_layers, model_dim=args.model_dim,
        num_heads=args.num_heads, num_kv_heads=args.num_kv_heads, mlp_mult=args.mlp_mult,
    ...
    if eval_model.qo_bank is not None:
        eval_model.qo_bank.data = eval_model.qo_bank.data.float()
    if eval_model.kv_bank is not None:
