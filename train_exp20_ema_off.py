...
class Hyperparameters:
    ...
    # exp20: Partial Weight-Tied 7L×2 with Loop Embedding.
    # num_layers = number of UNIQUE blocks (10 = 4 shared + 3 indep_p1 + 3 indep_p2).
    # Pass 1: shared[0..3] -> indep_p1[0..2]. Pass 2: shared[0..3] -> indep_p2[0..2].
    # Effective depth = 2 * (shared + indep) = 14 positions.
    num_layers = int(os.environ.get("NUM_LAYERS", 10))  # unique blocks: 4 shared + 3 indep_p1 + 3 indep_p2
    num_shared_layers = int(os.environ.get("NUM_SHARED_LAYERS", 4))
    num_indep_layers = int(os.environ.get("NUM_INDEP_LAYERS", 3))
    ...
    # exp20: 7L×2 partial weight-tied. Effective positions 0..13. VE at last 2 positions.
    ve_layers = os.environ.get("VE_LAYERS", "12,13")
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,loop_embed",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        # exp20: Partial Weight-Tied with Loop Embedding
        self.num_shared = num_shared_layers
        self.num_indep = num_indep_layers
        self.num_effective_per_pass = num_shared_layers + num_indep_layers  # 7
        self.num_skip_weights = self.num_effective_per_pass  # 7 skips for Pass 2
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, model_dim, dtype=torch.float32))
        # Loop embedding: pass indicator
        self.loop_embed = nn.Embedding(2, model_dim)
    ...
    def _init_weights(self) -> None:
        ...
        n = self.num_layers
        proj_scale = 1.0 / math.sqrt(2 * self.num_effective_per_pass)  # effective depth = 14
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        n = self.num_layers  # 10 unique blocks
        ns = self.num_shared  # 4
        ni = self.num_indep   # 3
        ...
        ve_cache: dict = {}
        # --- Pass 1 ---
        loop_signal = self.loop_embed.weight[0][None, None, :]
        x = x + loop_signal.to(dtype=x.dtype)
        # Shared layers (blocks[0..3], banks[0..3])
        for i in range(ns):
            ...
            if v0 is None and raw_v is not None:
                v0 = raw_v
            skips.append(x)
        # Pass1 independent layers (blocks[4..6], banks[4..6])
        for j in range(ni):
            i = ns + j
            pos = ns + j
            ...
            skips.append(x)
        # --- Pass 2 ---
        loop_signal_2 = self.loop_embed.weight[1][None, None, :]
        x = x + loop_signal_2.to(dtype=x.dtype)
        skip_idx = 0
        # Shared layers reused (blocks[0..3], banks[0..3])
        for i in range(ns):
            if skips:
                x = x + self.skip_weights[skip_idx].to(dtype=x.dtype)[None, None, :] * skips.pop()
                skip_idx += 1
            pos = ns + ni + i  # effective position 7, 8, 9, 10
            ve = self._get_ve(pos, input_ids, ve_cache)
            x, _ = self.blocks[i](x, x0,
                self.qo_bank[i], self.kv_bank[i], self.kv_bank[n + i],
                self.qo_bank[n + i], self.mlp_up_bank[i], self.mlp_down_bank[i],
                v_embed=ve, v0=v0)
        # Pass2 independent layers (blocks[7..9], banks[7..9])
        for j in range(ni):
            i = ns + ni + j
            if skips:
                x = x + self.skip_weights[skip_idx].to(dtype=x.dtype)[None, None, :] * skips.pop()
                skip_idx += 1
            pos = ns + ni + ns + j  # effective position 11, 12, 13
            ve = self._get_ve(pos, input_ids, ve_cache)
            x, _ = self.blocks[i](x, x0,
                self.qo_bank[i], self.kv_bank[i], self.kv_bank[n + i],
                self.qo_bank[n + i], self.mlp_up_bank[i], self.mlp_down_bank[i],
...
class _HessianGPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
                 ...
                 ve_enabled=False, ve_dim=128, ve_layers="12,13",
                 num_shared_layers=4, num_indep_layers=3):
        super().__init__()
        self.tie_embeddings = tie_embeddings
        self.logit_softcap = logit_softcap
        self.num_layers = num_layers
        ...
        self.num_shared = num_shared_layers
        self.num_indep = num_indep_layers
        self.num_effective_per_pass = num_shared_layers + num_indep_layers
        self.num_skip_weights = self.num_effective_per_pass
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, model_dim, dtype=torch.float32))
        # Loop embedding
        self.loop_embed = nn.Embedding(2, model_dim)
        nn.init.zeros_(self.loop_embed.weight)
    ...
    def forward(self, input_ids, target_ids):
        ...
        loop_signal = self.loop_embed.weight[0][None, None, :]
        x = x + loop_signal.to(dtype=x.dtype)
        # Shared layers (blocks[0..3])
        for i in range(ns):
            ve = self._get_ve(i, input_ids, ve_cache)
            x = self.blocks[i](x, x0, v_embed=ve)
            skips.append(x)
        # Pass1 independent layers (blocks[4..6])
        for j in range(ni):
            i = ns + j
            pos = ns + j
            ve = self._get_ve(pos, input_ids, ve_cache)
        ...
        x = x + loop_signal_2.to(dtype=x.dtype)
        skip_idx = 0
        # Shared layers reused (blocks[0..3])
        for i in range(ns):
            if skips:
                x = x + self.skip_weights[skip_idx].to(dtype=x.dtype)[None, None, :] * skips.pop()
                skip_idx += 1
            ...
            ve = self._get_ve(pos, input_ids, ve_cache)
            x = self.blocks[i](x, x0, v_embed=ve)
        # Pass2 independent layers (blocks[7..9])
        for j in range(ni):
            i = ns + ni + j
            if skips:
                x = x + self.skip_weights[skip_idx].to(dtype=x.dtype)[None, None, :] * skips.pop()
...
def main() -> None:
    ...
    scalar_params.append(base_model.loop_embed.weight)
    ...
    log0(f"weight_tied:num_unique_blocks={args.num_layers} num_shared={args.num_shared_layers} num_indep={args.num_indep_layers} effective_depth={2*(args.num_shared_layers+args.num_indep_layers)} model_dim={args.model_dim}")
    ...
    if args.lawa_enabled and len(lawa_queue) > 1:
        ...
        for name in swa_state:
            swa_state[name] = (swa_state[name] / swa_count).to(dtype=base_model.state_dict()[name].dtype)
        base_model.load_state_dict(swa_state, strict=True)
    else:
        log0("weight_averaging:none, using raw training weights")
    torch.cuda.synchronize()
    ...
    hessian_model = _HessianGPT(
        ...
        num_shared_layers=args.num_shared_layers, num_indep_layers=args.num_indep_layers,
    ).to(device).bfloat16()
    ...
    # Sort +-1 quantized values by their reconstruction error (scale^2),
