# exp16-1: 5L×3 Weight-Tied with Detach + Dynamic Blend (router grad-through)
# Changes from exp16:
#   - pass_router receives x0 WITHOUT detach, so router gets gradients from loss
#   - Pass 1+ block inputs remain detached (only router sees grad through x0)
...
class Block(nn.Module):
    def __init__(
        ...
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None, ln_scale_factor: float | None = None) -> tuple[Tensor, Tensor | None]:
        # exp12: caller may override the cached ln_scale_factor with an effective-depth
        # value that accounts for the 2-pass weight-tied structure.
        lnf = self.ln_scale_factor if ln_scale_factor is None else ln_scale_factor
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out, raw_v = self.attn(self.attn_norm(x_in) * lnf, q_w, k_w, v_w, out_w, v_embed=v_embed, v0=v0)
        x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
        x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(self.mlp_norm(x_out) * lnf, up_w, down_w)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        blend_logits = self.pass_router(x0)  # grad-through: router learns from loss via x0
...
class _HessianBlock(nn.Module):
    def __init__(self, dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, layer_idx=0, ln_scale=False):
        ...
        self.resid_mix = nn.Parameter(torch.stack((torch.ones(dim), torch.zeros(dim))).float())
        self.ln_scale_factor = 1.0 / math.sqrt(layer_idx + 1) if ln_scale else 1.0
    def forward(self, x, x0, v_embed=None, ln_scale_factor: float | None = None):
        lnf = self.ln_scale_factor if ln_scale_factor is None else ln_scale_factor
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out = self.attn(self.attn_norm(x_in) * lnf, v_embed=v_embed)
        x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
        x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(self.mlp_norm(x_out) * lnf)
        return x_out
...
def main() -> None:
    ...
    # SKIP_GPTQ: skip quantization for fast smoke tests on 4090
    if bool(int(os.environ.get("SKIP_GPTQ", "0"))):
