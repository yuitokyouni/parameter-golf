...
class GPT(nn.Module):
    def __init__(
        ...
        # exp20: Partial weight-tied 7L×2.
        ...
        # Loop embedding: distinguish Pass 1 vs Pass 2
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for j in range(ni):
            i = ns + j  # block/bank index 4, 5, 6
            pos = ns + j  # effective position 4, 5, 6
        ...
        for j in range(ni):
            i = ns + ni + j  # block/bank index 7, 8, 9
            if skips:
                x = x + self.skip_weights[skip_idx].to(dtype=x.dtype)[None, None, :] * skips.pop()
                skip_idx += 1
...
class _HessianGPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
        ...
        self.bigram = BigramHashEmbedding(bigram_vocab_size, bigram_dim, model_dim, trigram=bool(int(os.environ.get("TRIGRAM", "0")))) if bigram_vocab_size > 0 else None
        self.smear = SmearGate(model_dim)
        # Mirror the exp20 partial weight-tied loop structure of GPT.
