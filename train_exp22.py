# exp22: FiLM-only TTT — Test-Time Training for Weight-Tied Transformers
# Key idea: Instead of updating shared weights (which explodes with weight tying),
# update only per-pass FiLM parameters (γ, β) at eval time.
# This resolves the TTT × Weight-Tied incompatibility reported by prior work.
# FiLM params: ~16K (0.03MB) — negligible size impact, low overfitting risk.
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,film_gamma,film_beta",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        # FiLM conditioning: per-pass × per-layer affine transform (γ, β)
        self.num_passes = 2  # encoder + decoder
        self.film_gamma = nn.ParameterList([
            nn.Parameter(torch.ones(model_dim))
            for _ in range(self.num_passes * num_layers)
        ])
        self.film_beta = nn.ParameterList([
            nn.Parameter(torch.zeros(model_dim))
            for _ in range(self.num_passes * num_layers)
        ])
        self._init_weights()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            if v0 is None and raw_v is not None:
                v0 = raw_v
            # FiLM conditioning (pass 0, layer i)
            film_idx = 0 * n + i
            x = self.film_gamma[film_idx].to(dtype=x.dtype)[None, None, :] * x + self.film_beta[film_idx].to(dtype=x.dtype)[None, None, :]
            skips.append(x)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[i](x, x0,
                ...
                self.qo_bank[n + i], self.mlp_up_bank[i], self.mlp_down_bank[i],
                v_embed=ve, v0=v0)
            # FiLM conditioning (pass 1, layer i)
            film_idx = 1 * n + i
            x = self.film_gamma[film_idx].to(dtype=x.dtype)[None, None, :] * x + self.film_beta[film_idx].to(dtype=x.dtype)[None, None, :]
        x = self.final_norm(x)
...
# --- FiLM-only TTT evaluation ---

def eval_val_film_ttt(
    args: Hyperparameters,
    base_model: nn.Module,
    rank: int,
    world_size: int,
    ...
    """FiLM-only TTT evaluation.
    Score before update (Legal TTT): each chunk is scored with no_grad first,
    then FiLM parameters are updated via SGD on that chunk's loss.
    FiLM is reset to trained values at each document boundary."""
    seq_len = eval_seq_len or args.train_seq_len
    chunk_size = args.ttt_chunk_size
    ttt_lr = args.ttt_lr
    ttt_steps = args.ttt_steps

    # Save trained FiLM as initial values
    film_init: dict[str, Tensor] = {}
    for name, p in base_model.named_parameters():
        if 'film_' in name:
            film_init[name] = p.detach().clone()

    # Treat entire val set as one "document" (no document boundaries in this dataset)
    total_tokens = val_tokens.numel() - 1
    # Split across ranks
    ...
    # Reset FiLM to initial values
    with torch.no_grad():
        for name, p in base_model.named_parameters():
            if 'film_' in name:
                p.copy_(film_init[name])

    # Set up FiLM-only optimizer
    film_params = [p for name, p in base_model.named_parameters() if 'film_' in name]
    ttt_optimizer = torch.optim.SGD(film_params, lr=ttt_lr)

    # Freeze everything except FiLM
    grad_state: dict[str, bool] = {}
    for name, p in base_model.named_parameters():
        grad_state[name] = p.requires_grad
        p.requires_grad_('film_' in name)

    # Process chunks sequentially
    num_chunks = (rank_tokens + chunk_size - 1) // chunk_size
    for ci in range(num_chunks):
        c_start = rank_start + ci * chunk_size
        c_end = min(rank_start + (ci + 1) * chunk_size, rank_end)
        c_len = c_end - c_start
        if c_len <= 0:
            break

        chunk = val_tokens[c_start:c_end + 1].to(device=device, dtype=torch.int64)
        chunk_x = chunk[:-1].unsqueeze(0)  # [1, c_len]
        chunk_y = chunk[1:].unsqueeze(0)   # [1, c_len]

        # Step 1: Score before update (Legal TTT)
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                logits = base_model.forward_logits(chunk_x)
            nll = F.cross_entropy(
                logits.reshape(-1, logits.size(-1)).float(),
                chunk_y.reshape(-1),
                reduction="none",
            ...
            token_count += float(c_len)
            prev_ids = chunk_x.reshape(-1)
            tgt_ids = chunk_y.reshape(-1)
        ...
        # Step 2: Update FiLM parameters only
        for _ in range(ttt_steps):
            ttt_optimizer.zero_grad()
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = base_model(chunk_x, chunk_y)
            loss.backward()
            ttt_optimizer.step()

    # Restore requires_grad state
    for name, p in base_model.named_parameters():
        p.requires_grad_(grad_state[name])

    # Restore FiLM to initial values
    with torch.no_grad():
        for name, p in base_model.named_parameters():
            if 'film_' in name:
                p.copy_(film_init[name])
...
def main() -> None:
    ...
    # FiLM parameters (γ, β) in scalar group
    for p in base_model.film_gamma:
        scalar_params.append(p)
    for p in base_model.film_beta:
        scalar_params.append(p)
    ...
    film_param_count = sum(p.numel() for p in base_model.film_gamma) + sum(p.numel() for p in base_model.film_beta)
    log0(f"film_ttt:enabled={args.ttt_enabled} lr={args.ttt_lr} chunk_size={args.ttt_chunk_size}")
    log0(f"film_params:{film_param_count} ({film_param_count * 4 / 1024:.1f}KB)")
    ...
    sw_val_bpb = None  # set by sliding window eval if it runs
    ...
    # FiLM-only TTT evaluation (if enabled)
    if args.ttt_enabled:
        log0("ttt:starting FiLM-only TTT evaluation...")
        # Use uncompiled model for TTT (dynamic graph due to gradient updates)
        torch.cuda.synchronize()
        t_ttt = time.perf_counter()
        # Record pre-TTT bpb (use the sliding window result if available, else int6 roundtrip)
        pre_ttt_bpb = sw_val_bpb if sw_val_bpb is not None else q_val_bpb
        ttt_val_loss, ttt_val_bpb = eval_val_film_ttt(
            args, eval_model, rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            eval_seq_len=effective_eval_seq_len,
        )
        torch.cuda.synchronize()
        ttt_gain = pre_ttt_bpb - ttt_val_bpb
        log0(
            f"ttt:pre_val_bpb={pre_ttt_bpb:.4f} post_val_bpb={ttt_val_bpb:.4f} gain={ttt_gain:.4f} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_ttt):.0f}ms"
        )
        log0(f"ttt:post_val_bpb_exact={ttt_val_bpb:.8f}")
    if distributed:
