# exp18: Hybrid GDN — 5 GDN blocks + 1 SWA block (last)
...
class Hyperparameters:
    ...
    xsa_last_n = int(os.environ.get("XSA_LAST_N", 1))  # XSA only on the 1 SWA block
...
class GPT(nn.Module):
    def __init__(
        ...
        # Parse attention types per block
        ...
        self.blocks = nn.ModuleList([
            Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                  ...
                  gated_attention=gated_attention, value_residual=value_residual,
                  attention_type=self.block_attn_types[i], gdn_num_heads=gdn_num_heads)
            for i in range(num_layers)
        ])
        # RoPE only for SWA blocks
        if rope_dims > 0:
            head_dim = model_dim // num_heads
    ...
    def _init_weights(self) -> None:
        ...
        for j, i in enumerate(self.swa_indices):
            nn.init.orthogonal_(self.qo_bank.data[j], gain=1.0)        # Q
            nn.init.zeros_(self.qo_bank.data[num_swa + j])              # Out (zero init)
            nn.init.orthogonal_(self.kv_bank.data[j], gain=1.0)        # K
            nn.init.orthogonal_(self.kv_bank.data[num_swa + j], gain=1.0)  # V
...
def _rebank_state_dict(sd: dict[str, Tensor], num_layers: int, template_sd: dict[str, Tensor], swa_indices: list[int] | None = None) -> dict[str, Tensor]:
    ...
    consumed = set()
    for j, i in enumerate(swa_list):
        for key, slot in [(f"blocks.{i}.attn.c_q.weight", ('qo', j)),
                          (f"blocks.{i}.attn.proj.weight", ('qo', num_swa + j)),
                          (f"blocks.{i}.attn.c_k.weight", ('kv', j)),
                          (f"blocks.{i}.attn.c_v.weight", ('kv', num_swa + j))]:
            if key in sd:
                if slot[0] == 'qo': qo_slices[slot[1]] = sd[key]
                else: kv_slices[slot[1]] = sd[key]
                consumed.add(key)
    for i in range(n):
...
class _HessianBlock(nn.Module):
    def __init__(self, dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                 layer_idx=0, ln_scale=False, attention_type='swa', gdn_num_heads=8):
        super().__init__()
        self.attention_type = attention_type
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
